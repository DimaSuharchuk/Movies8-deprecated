...
class EntityFinder {
  ...
  public function findParagraphEpisode(int $parent_tv_nid, int $season_number, int $episode_number): self {
    $season = $this->findParagraphSeason($parent_tv_nid, $season_number)
      ->reduce()
      ->execute();

    $this->findParagraphs()->byBundle(IMDbQueueItemRequestType::EPISODE);
    $this->addCondition('parent_id', $season->id());
  ...
  public function execute() {
    $entities = $this->loadByProperties($this->search_values);

    $this->storage = NULL;
    $this->search_values = [];
    $this->limit = 0;

    return $this->reduce ? reset($entities) : $entities;
  }
   ...
   */
  private function loadByProperties(array $values = []): array {
    if ($this->storage) {
